...
export class ValidConfig {
    ...
    static onlyEnglishLatersAndNumbers(errorMessage = "This field can't be negative."): ValidatorFn {

        const EnglishLattersChecker = /^[a-z0-9]*$/i;
        return formControl => !EnglishLattersChecker.test(formControl.value) ?  { [ValidConfig.key]: errorMessage } : null ;
    }

    static onlyEnglishLaters(errorMessage = "This field can't be negative."): ValidatorFn {

        const EnglishLattersChecker = /^[a-z]*$/i;
        return formControl => !EnglishLattersChecker.test(formControl.value) ?  { [ValidConfig.key]: errorMessage } : null ;
    }
}